...
class Position {
    ...
    async getDetailPosition(username) 
    {
        var x = new Promise( function (fulfilled, rejected) {
            PositionModel.findOne({username: username, is_actived: true, approved: true}, function(err, data) {
                if(err) rejected(err)
    ...
    async save() 
    {
        ...
        try {
            ...
            return new Promise( function (fulfilled, rejected) {
                PositionModel.findOneAndUpdate({_id: currentId}, params, { upsert: true, new: true }, (err, doc) => {
                    if(err) {
                        rejected(err)
                    }
